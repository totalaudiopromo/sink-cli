...
> = {
  ...
  'fuzzy-name': (records, config) => fuzzyMatch(records, config.rinse.fuzzyThreshold),
...
export async function rinse(
  ...
  const strategies = (
    config.rinse.strategies ?? ['exact-email', 'fuzzy-name', 'cross-field']
  ...
  return current.map((r) => ({
    ...r,
    phases: r.phases.includes('rinse') ? r.phases : [...r.phases, 'rinse' as const],
  }))
}
