 ...
 */
export function fuzzyMatch(records: SinkRecord[], threshold = 0.92): SinkRecord[] {
  ...
  for (let i = 0; i < result.length; i++) {
    ...
    const email = result[i].scrub?.email.normalised || result[i].raw.email
    if (!email) continue
    ...
    for (let a = 0; a < indices.length; a++) {
      ...
      for (let b = a + 1; b < indices.length; b++) {
        ...
        if (similarity >= threshold) {
          ...
          const countA = Object.values(result[indices[a]].raw).filter(Boolean).length
          const countB = Object.values(result[indices[b]].raw).filter(Boolean).length
          const dupIdx = countB >= countA ? indices[a] : indices[b]
          const canonIdx = dupIdx === indices[a] ? indices[b] : indices[a]
          const canonEmail =
            result[canonIdx].scrub?.email.normalised || result[canonIdx].raw.email || ''
          result[dupIdx] = {
            ...result[dupIdx],
            rinse: {
              duplicate: true,
