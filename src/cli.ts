...
async function runVerify(email: string): Promise<void> {
  ...
  if (result.corrected) {
    step(`Corrected:   ${chalk.yellow(result.original ?? '')} ${chalk.dim('→')} ${chalk.green(result.suggested ?? '')}`);
  }
...
async function runFileStats(filePath: string): Promise<void> {
  ...
  const score = total > 0 ? Math.round(((stats.scrub.valid + stats.scrub.risky * 0.5) / total) * 100) : 0;
  const scoreColour = score >= 80 ? chalk.green : score >= 60 ? chalk.yellow : chalk.red;

  step(`${records.length.toLocaleString('en-GB')} contacts, ${total.toLocaleString('en-GB')} with email`);
  blank();
  step(`Quality score: ${scoreColour(score + '%')}`);
  blank();
  validationRow('ok', 'Valid', stats.scrub.valid, '');
  validationRow(stats.scrub.risky > 0 ? 'warn' : 'ok', 'Risky', stats.scrub.risky, '(role/catch-all)');
